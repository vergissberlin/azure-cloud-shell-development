...
| `APIGEE_HELM_CHARTS_HOME`  | Local directory where Hybrid Helm charts are stored. `cshell setup` records the current working directory here and installs a small Bash export snippet (`~/.cshell-helm-charts-home.sh`) sourced from `~/.bashrc` for new sessions.                                                                                         |
