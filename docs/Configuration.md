...
| `APIGEE_HELM_CHARTS_HOME`  | Local directory where Hybrid Helm charts are stored (default **`~/apigee-hybrid/helm-charts`**, created by `cshell setup` with `mkdir -p`). After **`cshell setup`**, **`cshell init`**, or **`cshell config set`**, cshell regenerates **`~/.cshell-env-exports.sh`** and adds hooks so **new Bash sessions** load it (**`~/.bashrc`**, **`~/.profile`**, **`~/.bash_profile`** when present). **`cshell hybrid`** and **`cshell hybrid --export`** refresh the snippet only — they **do not** modify those hooks; load variables in the **current** shell with **`eval "$(cshell hybrid --export --print)"`** or **`. ~/.cshell-env-exports.sh`**. Requires **bash 4+** for snippet generation. The snippet file is not `+x` on purpose.                                                                                         |
